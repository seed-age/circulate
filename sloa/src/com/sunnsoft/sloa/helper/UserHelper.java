...
public class UserHelper extends HelperAbstract<User>{
	...
	protected UserJSONHelper jsonHelper;
	...
	static{
		...
		titles.put("accountName", "登录名");
		titles.put("nickName", "昵称");
		titles.put("createTime", "创建日期");
		titles.put("userPassword", "密码");
		titles.put("admin", "是否管理员");
		titles.put("enabled", "是否启用");
		titles.put("lastLogin", "最后登录时间");
		titles.put("priority", "优先级");
		titles.put("softDelete", "是否软删除");
		titles.put("accessToken", "登录令牌");
		titles.put("accessTokenExpire", "令牌失效时间");
	 ...
	 * @return
	 */
	public UserHelper ignoreEmptyValueCondiction(){
		this.ignoreEmpty = true;
		return this;
	}
	
	public UserHelper(UserService service,String alias) {
		super();
	 ...
	 * @return
	 */
	public UserHelper setPageSize(int pageSize){
	 ...
	 * @return
	 */
	public UserHelper newOne(){
		if(StringUtils.isNotEmpty(alias)){
			throw new HelperException("别称类不可以newDc!");
		}
		return new UserHelper(service,"");
	}
	/**
	 * 为Helper类注入一个附属dc
	 * @return
	 */
	public UserHelper setDc(DetachedCriteria dc){
		if(StringUtils.isNotEmpty(alias)){
	 ...
	 * @return
	 */
	public UserHelper distinctEntity(){
	 ...
	 * @return
	 */
	public UserJSONHelper json(){
		if(jsonHelper != null) {
			return jsonHelper;
		}
		return new UserJSONHelper(this);
	}
	...
	@Override
	public PageList<User> listPage(int page,int pageSize){
		if(StringUtils.isNotEmpty(alias)){
	...
	@Override
	public PageScroll<User> listPageScroll(int page,int pageSize){
		if(StringUtils.isNotEmpty(alias)){
			throw new HelperException("别称类不可以调用listPage!");
		}
		while(!junctions.empty()){
	...
	@Override
	public PageList<User> listPage(int page){
		if(StringUtils.isNotEmpty(alias)){
			throw new HelperException("别称类不可以调用listPage!");
		}
		while(!junctions.empty()){
	...
	@Override
	public List<User> each(final Each<User> each){
		if(StringUtils.isNotEmpty(alias)){
			throw new HelperException("别称类不可以调用each!");
		}
		return (List<User>)this.service.executeTransactional(new TransactionalCallBack(){

			@Override
			public Object execute(IService service) {
				List<User> list = list();
				for (Iterator<User> iterator = list.iterator(); iterator.hasNext();) {
					User bean = iterator.next();
					User oldBean = beforeEachUpdate(bean);
					each.each(bean, list);
	...
	@Override
	public long scrollResult(final ScrollEach<User> each){
		if(StringUtils.isNotEmpty(alias)){
	...
	@Override
	public PageList<User> eachInPage(final int page,final Each<User> each){
		return eachInPage(page,pageSize,each);
	}
	
	@Override
	public PageScroll<User> eachInPageScroll(final int page,final Each<User> each){
		return eachInPageScroll(page,pageSize,each);
	}
	
	@Override
	public PageList<User> eachInPage(final int page,final int pageSize,final Each<User> each){
		if(StringUtils.isNotEmpty(alias)){
			throw new HelperException("别称类不可以调用eachInPage!");
		}
		return (PageList<User>) this.service.executeTransactional(new TransactionalCallBack(){

			@Override
			public Object execute(IService service) {
				PageList<User> pageList = listPage(page,pageSize);
	...
	@Override
	public PageScroll<User> eachInPageScroll(final int page,final int pageSize,final Each<User> each){
		if(StringUtils.isNotEmpty(alias)){
			throw new HelperException("别称类不可以调用eachInPage!");
		}
		return (PageScroll<User>) this.service.executeTransactional(new TransactionalCallBack(){

			@Override
			public Object execute(IService service) {
				PageScroll<User> pageScroll = listPageScroll(page,pageSize);
				List<User> list = pageScroll.getList();
				for (Iterator<User> iterator = list.iterator(); iterator.hasNext();) {
					User bean = iterator.next();
					User oldBean = beforeEachUpdate(bean);
					each.each(bean, list);
	 ...
	 * @return
	 */
	public UserHelper limit(int limit){
		if(StringUtils.isNotEmpty(alias)){
	 ...
	 * @return
	 */
	public UserHelper limit(int start ,int limit){
		if(StringUtils.isNotEmpty(alias)){
			throw new HelperException("别称类不可以调用limit!");
		}
		this.limit = limit;
	...
	//== 关联关系处理  ==//
	/**
	 * inner join UserInfo ，进入关联关系实体设置查询条件。
	 ...
	 */
	public UserInfoHelper enterUserInfo(){
		return this.enterUserInfoWithJoinType(0);
	}
	
	/**
	 * left join UserInfo ，进入关联关系实体设置查询条件。
	 * 注意，此时不能进行分页查询（不能用listPage,listPageJson等方法），得到每一页的实体数量可能是不正确的。
	 * 如果实现需要关联查询并分页，请用纯SQL操作，或用冗余表和字段的方式把关联关系合并到一个表，必要的时候可以考虑编写触发器，或将此类信息复制一份到mongdb，然后进行查询。
	 */
	public UserInfoHelper leftJoinUserInfo(){
		return this.enterUserInfoWithJoinType(1);
	}
	
	/**
	 * full join UserInfo ，进入关联关系实体设置查询条件,full join 查询较少用到。
	 * 注意，此时不能进行分页查询（不能用listPage,listPageJson等方法），得到每一页的实体数量可能是不正确的。
	 * 如果实现需要关联查询并分页，请用纯SQL操作，或用冗余表和字段的方式把关联关系合并到一个表，必要的时候可以考虑编写触发器，或将此类信息复制一份到mongdb，然后进行查询。
	 */
	public UserInfoHelper fullJoinUserInfo(){
		return this.enterUserInfoWithJoinType(2);
	}
	
	/**
	 * 创建UserInfo别称userInfo查询条件，并根据joinType参数设置jion类型。
	 * 本方法只被内部3种join类型方法调用。
	 * 
	 * @param joinType
	 * @return
	 */
	private UserInfoHelper enterUserInfoWithJoinType(int joinType){
		UserInfoService userInfoService = (UserInfoService) this.service.getBean("userInfoService");
		String enterAlias = this.alias+"_userInfo";//本级别称加上"_"加上属性名来创建子别称
		boolean notExists = this.aliases.add(enterAlias);
		if(notExists){
			switch (joinType) {
			case 1:
				dc.createAlias(this.alias+("".equals(this.alias)?"":".")+"userInfo", enterAlias,CriteriaSpecification.LEFT_JOIN);//以left join 方式创建别称。
				break;
				
			case 2:
				dc.createAlias(this.alias+("".equals(this.alias)?"":".")+"userInfo", enterAlias,CriteriaSpecification.FULL_JOIN);//以full join 方式创建别称。
				break;
				
			default:
				dc.createAlias(this.alias+("".equals(this.alias)?"":".")+"userInfo", enterAlias);//默认以inner join 方式创建别称。
				break;
			}
		}
		UserInfoHelper h = new UserInfoHelper(userInfoService,enterAlias);//用别称创建别称helper
	...
	/**
	 * 从UserInfo别称userInfo返回
	 * @return
	 */
	public UserInfoHelper back2UserInfo(){
		if(this.helpers.empty()){
			throw new HelperException("无法回到helper类:null");
		}
		HelperAbstract<UserInfo> h =  this.helpers.pop();
	...
	*/
	...
	public UserProperty<Long> getUserId(){
		return new UserProperty<Long>("userId",this,Long.class);
	}
	...
	*/
	...
	public UserStringProperty getAccountName(){
		return new UserStringProperty("accountName",this);
	}
	...
	*/
	...
	public UserStringProperty getNickName(){
		return new UserStringProperty("nickName",this);
	}
	...
	*/
	...
	public UserDateProperty getCreateTime(){
		return new UserDateProperty("createTime",this);
	}
	...
	*/
	...
	public UserStringProperty getUserPassword(){
		return new UserStringProperty("userPassword",this);
	}
	...
	*/
	...
	public UserProperty<Boolean> getAdmin(){
		return new UserProperty<Boolean>("admin",this,ClassUtils.wrapperToPrimitive(Boolean.class));
	}
	...
	*/
	...
	public UserProperty<Boolean> getEnabled(){
		return new UserProperty<Boolean>("enabled",this,ClassUtils.wrapperToPrimitive(Boolean.class));
	}
	...
	*/
	...
	public UserDateProperty getLastLogin(){
		return new UserDateProperty("lastLogin",this);
	}
	...
	*/
	...
	public UserProperty<Integer> getPriority(){
		return new UserProperty<Integer>("priority",this,ClassUtils.wrapperToPrimitive(Integer.class));
	}
	...
	*/
	...
	public UserProperty<Boolean> getSoftDelete(){
		return new UserProperty<Boolean>("softDelete",this,ClassUtils.wrapperToPrimitive(Boolean.class));
	}
	...
	*/
	...
	public UserStringProperty getAccessToken(){
		return new UserStringProperty("accessToken",this);
	}
	...
	*/
	...
	public UserDateProperty getAccessTokenExpire(){
		return new UserDateProperty("accessTokenExpire",this);
	}
	
	
	

	public UserListProperty getRoles(){
		return new UserListProperty("roles",this);
	}
	 ...
	 * @return
	 */
	public UserHelper inAnywhere(String value,boolean eq){
		this.startOr();
		if (eq) {
			getAccountName().Eq(value);
			getNickName().Eq(value);
			getUserPassword().Eq(value);
			getAccessToken().Eq(value);
		} else {
			getAccountName().Like(value);
			getNickName().Like(value);
			getUserPassword().Like(value);
			getAccessToken().Like(value);
		}
		this.stopOr();
		return this;
	}
	...
	private UpdateListener<User> updateListener;
	/**
	 * 监听调用each/eachInPage和scrollResult方法修改的bean。
	 * @param listener
	 * @return
	 */
	public UserHelper onUpdate(UpdateListener<User> listener){
	 ...
	 * @return
	 */
	public UserHelper sleepInteval(int interval){
	 ...
	 * @return
	 */
	public UserHelper sort(String dir,String propName){
	...
	private void afterEachUpdate(User bean, User oldBean) {
		if(updateListener != null){
			updateListener.each(oldBean, bean);
		}
	}
	
	private User beforeEachUpdate(User bean) {
		User oldBean = null;
		if(updateListener != null){
			oldBean = new User();
			try {
