...
/**
 * 已发/已收传阅 -- 点击编辑 -- 选择关注: 表示批量或单个关注传阅，点击进行关注.
 * 
 * @author chenjian
 *
 */
public class BatchInsertAttention extends BaseParameter {
	...
	public String execute() throws Exception {
		...
		// 根据传阅ID查询传阅数据
		List<Mail> ids = Services.getMailService().findByIds(mailId);
				...
				if (attention) { // true 为关注
					...
					if (mail.getUserId() == userId.longValue()) {
						...
						if (mail.getAttention().equals(false)) { // false 表示 未关注
							...
							Services.getUserCollectionService().createHelper().bean().create().setMail(mail)
									...
									.setDifferentiate(name).insertUnique();
					...
					if (mail.getUserId() == userId.longValue()) {
						...
						if (mail.getAttention().equals(true)) { // true 表示 已关注
							// 查询出该关注记录
							Services.getUserCollectionService().createHelper().getMail()
									.Eq(mail).getUserId().Eq(userId).getDifferentiate().Eq(name).delete();
				...
				if (attention) { // true 为关注
					...
					if (mail.getUserId() == userId.longValue()) {
						...
						if (mail.getAttention().equals(true)) { // true 表示 已关注
							// 查询出该关注记录
						    Services.getUserCollectionService().createHelper().getMail()
									.Eq(mail).getUserId().Eq(userId).getDifferentiate().Eq(deleteName).delete();
								// 设置是否关注这个字段为 false 表示关注
								mail.setAttention(attention);
								// 更新
								Services.getMailService().update(mail);
				...
				for (Receive receive : receives) {
					...
					if (attention) { // true 表示 关注
						...
						if (receive.getUserId() == userId.longValue()) {
							// 判断该传阅有没有被关注
							if (receive.getReceiveAttention().equals(false)) { // true 表示 已关注
								// 新增一条关注记录
								Services.getUserCollectionService().createHelper().bean().create().setMail(mail)
						...
						if (receive.getUserId() == userId.longValue()) {
							...
							if (receive.getReceiveAttention().equals(true)) { // true 表示 已关注
								// 查询出该关注记录
								Services.getUserCollectionService().createHelper().getMail().Eq(mail).getUserId()
										.Eq(userId).getDifferentiate().Eq(reName).delete();
								// 设置是否关注这个字段为 false 表示关注
