...
(function (factory) {
  ...
  $.widget('mf.manifest', {
    ...
    _create: function () {
      ...
      self.$search = $('<span class="add-search" /></span>');              
    ...
    add: function (data, $mpItem, clearInputValue, initial) {
      var self = this,
          ...
          buildItem = function(formatDisplay, formatRemove, formatValue) {
            ...
            var dataInput = self.$list.find('input');
            for(var i=0;i<dataInput.length;i++){
              if(formatValue.userId === $(dataInput[i]).data('userid')){
                return false;
              }
              
            }
            $item.html(formatValue.lastName);
            // Format the HTML display of the remove icon.
            $remove.html(formatRemove);
            // Format the hidden value to be submitted for the item.
            $value.attr({
              'data-userid':formatValue.userId
              // 'data-receiveWorkCode':formatValue.receiveWorkCode,
              // 'data-receiveLastName':formatValue.receiveLastName,
              // 'data-receiveLoginId':formatValue.receiveLoginId,
              // 'data-subcompanyName':formatValue.subcompanyName,
              // 'data-departmentName':formatValue.departmentName
            });
      ...
      for (var i = 0, length = values.length; i < length; i++) {
        ...
        $remove = $('<a href="#" class="mf_remove layui-icon" title="删除" />');
        $value = $('<input type="hidden" class="mf_value" />');
        // $item.html(data.receiveLastName);
        // debugger;
