...
public interface UserMssageService extends IService<UserMssage, Long> {
	/**
	 * 获得所有数据记录
	 ...
	 * @return vo
 	 */
	public UserMssage findById(Long id);
	/**
	 * 根据ID列表查找vo 列表,不包括找不到的对象。
	 * @param ids
	 * @return vo 列表
 	 */
	public List<UserMssage> findByIds(Long[] ids);
	/**
	 * 根据ID列表查找vo 列表，找不到的对象用null代替。
	 * @param ids
	 * @return vo 列表
 	 */
	public List<UserMssage> findByIdsIncludeNull(Long[] ids);
	/**
	 * 根据某个属性的值查找vo
	 * @param propName
	 * @param value
	 ...
	 * @return 结果"卷轴分页"对象 {@link PageScroll}
	 */
	public PageScroll<UserMssage> findByDetachedCriteriaByDefaultPageScroll(DetachedCriteria dc, int limit);
	/**
