...
public class MailProperty<T> {
	 ...
	 * @param helper
	 */
	public MailProperty(String propertyName, MailHelper helper,Class<T> type) {
		super();
		this.helper = helper;
	 ...
	 * @return
	 */
	public MailHelper In(Collection<T> values){
	 ...
	 * @return
	 */
	public MailHelper NotIn(T... values){
		if(helper.ignoreEmpty && (values == null || values.length == 0 )){
			return helper;
		}
	 ...
	 * @return
	 */
	public MailHelper NotIn(Collection<T> values){
		if(helper.ignoreEmpty && (values == null || values.isEmpty() )){
			return helper;
		}
		this.add(Restrictions.not(Restrictions.in(propertyName, values)));
	 ...
	 * @return
	 */
	public MailHelper EqOrNull(T value){
		if(value == null){
	 ...
	 * @return
	 */
	public MailHelper NeOrNull(T value){
		if(value == null){
			throw new HelperException("value不能为null");
		}
	...
	public void setValue(final T value){
		...
		try {
			final Method m = Mail.class.getMethod(setMethodName, this.type);
