...
import com.sunnsoft.sloa.service.TagService;
...
public class TagHelper extends HelperAbstract<Tag>{
	...
	protected TagJSONHelper jsonHelper;
	...
	static{
		titles.put("tagId", "标签iid");
		titles.put("tagType", "类型");
		titles.put("storageType", "存放的方式");
		titles.put("selfName", "自定义的标识");
		titles.put("companyName", "公司名称");
		titles.put("labelStatus", "状态");
	 ...
	 * @return
	 */
	public TagHelper ignoreEmptyValueCondiction(){
		this.ignoreEmpty = true;
		return this;
	}
	
	public TagHelper(TagService service,String alias) {
		super();
	 ...
	 * @return
	 */
	public TagHelper setPageSize(int pageSize){
	 ...
	 * @return
	 */
	public TagHelper newOne(){
		if(StringUtils.isNotEmpty(alias)){
			throw new HelperException("别称类不可以newDc!");
		}
		return new TagHelper(service,"");
	}
	/**
	 * 为Helper类注入一个附属dc
	 * @return
	 */
	public TagHelper setDc(DetachedCriteria dc){
		if(StringUtils.isNotEmpty(alias)){
	 ...
	 * @return
	 */
	public TagHelper distinctEntity(){
		BaseDAO.distinctRootWithPk(dc,"tagId");
	 ...
	 * @return
	 */
	public TagJSONHelper json(){
		if(jsonHelper != null) {
			return jsonHelper;
		}
		return new TagJSONHelper(this);
	}
	 ...
	 * @return
	 */
	public List<Tag> delete(){
		List<Tag> deleteList = list();
	...
	@Override
	public PageList<Tag> listPage(int page,int pageSize){
		if(StringUtils.isNotEmpty(alias)){
	...
	@Override
	public PageScroll<Tag> listPageScroll(int page,int pageSize){
		if(StringUtils.isNotEmpty(alias)){
			throw new HelperException("别称类不可以调用listPage!");
		}
		while(!junctions.empty()){
	...
	@Override
	public PageList<Tag> listPage(int page){
		if(StringUtils.isNotEmpty(alias)){
			throw new HelperException("别称类不可以调用listPage!");
		}
		while(!junctions.empty()){
	...
	@Override
	public List<Tag> each(final Each<Tag> each){
		if(StringUtils.isNotEmpty(alias)){
			throw new HelperException("别称类不可以调用each!");
		}
		return (List<Tag>)this.service.executeTransactional(new TransactionalCallBack(){

			@Override
			public Object execute(IService service) {
				List<Tag> list = list();
				for (Iterator<Tag> iterator = list.iterator(); iterator.hasNext();) {
					Tag bean = iterator.next();
					Tag oldBean = beforeEachUpdate(bean);
					each.each(bean, list);
	...
	@Override
	public long scrollResult(final ScrollEach<Tag> each){
		if(StringUtils.isNotEmpty(alias)){
	...
	@Override
	public PageList<Tag> eachInPage(final int page,final Each<Tag> each){
		return eachInPage(page,pageSize,each);
	}
	
	@Override
	public PageScroll<Tag> eachInPageScroll(final int page,final Each<Tag> each){
		return eachInPageScroll(page,pageSize,each);
	}
	
	@Override
	public PageList<Tag> eachInPage(final int page,final int pageSize,final Each<Tag> each){
		if(StringUtils.isNotEmpty(alias)){
			throw new HelperException("别称类不可以调用eachInPage!");
		}
		return (PageList<Tag>) this.service.executeTransactional(new TransactionalCallBack(){

			@Override
			public Object execute(IService service) {
				PageList<Tag> pageList = listPage(page,pageSize);
				List<Tag> list = pageList.getList();
				for (Iterator<Tag> iterator = list.iterator(); iterator.hasNext();) {
					Tag bean = iterator.next();
					Tag oldBean = beforeEachUpdate(bean);
					each.each(bean, list);
	...
	@Override
	public PageScroll<Tag> eachInPageScroll(final int page,final int pageSize,final Each<Tag> each){
		if(StringUtils.isNotEmpty(alias)){
			throw new HelperException("别称类不可以调用eachInPage!");
		}
		return (PageScroll<Tag>) this.service.executeTransactional(new TransactionalCallBack(){

			@Override
			public Object execute(IService service) {
				PageScroll<Tag> pageScroll = listPageScroll(page,pageSize);
				List<Tag> list = pageScroll.getList();
				for (Iterator<Tag> iterator = list.iterator(); iterator.hasNext();) {
					Tag bean = iterator.next();
					Tag oldBean = beforeEachUpdate(bean);
					each.each(bean, list);
	 ...
	 * @return
	 */
	public TagHelper limit(int limit){
		if(StringUtils.isNotEmpty(alias)){
	 ...
	 * @return
	 */
	public TagHelper limit(int start ,int limit){
		if(StringUtils.isNotEmpty(alias)){
			throw new HelperException("别称类不可以调用limit!");
		}
		this.limit = limit;
	...
	*/
	...
	public TagProperty<Long> getTagId(){
		return new TagProperty<Long>("tagId",this,Long.class);
	}
	...
	*/
	...
	public TagStringProperty getTagType(){
		return new TagStringProperty("tagType",this);
	}
	...
	*/
	...
	public TagStringProperty getStorageType(){
		return new TagStringProperty("storageType",this);
	}
	...
	*/
	...
	public TagStringProperty getSelfName(){
		return new TagStringProperty("selfName",this);
	}
	...
	*/
	...
	public TagStringProperty getCompanyName(){
		return new TagStringProperty("companyName",this);
	}
	...
	*/
	...
	public TagProperty<Integer> getLabelStatus(){
		return new TagProperty<Integer>("labelStatus",this,Integer.class);
	}
	 ...
	 * @return
	 */
	public TagHelper inAnywhere(String value,boolean eq){
		this.startOr();
		if (eq) {
			getTagType().Eq(value);
			getStorageType().Eq(value);
			getSelfName().Eq(value);
			getCompanyName().Eq(value);
		} else {
			getTagType().Like(value);
			getStorageType().Like(value);
			getSelfName().Like(value);
			getCompanyName().Like(value);
		}
		this.stopOr();
		return this;
	}
	...
	private UpdateListener<Tag> updateListener;
	/**
	 * 监听调用each/eachInPage和scrollResult方法修改的bean。
	 * @param listener
	 * @return
	 */
	public TagHelper onUpdate(UpdateListener<Tag> listener){
	 ...
	 * @return
	 */
	public TagHelper sleepInteval(int interval){
	 ...
	 * @return
	 */
	public TagHelper sort(String dir,String propName){
	...
	private void afterEachUpdate(Tag bean, Tag oldBean) {
		if(updateListener != null){
			updateListener.each(oldBean, bean);
		}
	}
	
	private Tag beforeEachUpdate(Tag bean) {
		Tag oldBean = null;
		if(updateListener != null){
			oldBean = new Tag();
			try {
