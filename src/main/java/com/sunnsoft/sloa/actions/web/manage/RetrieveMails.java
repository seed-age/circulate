package com.sunnsoft.sloa.actions.web.manage;
 ...
 *
 */
public class RetrieveMails extends BaseParameter {
...
//            ReceiveHelper helper = Services.getReceiveService().createHelper().getUserId().Eq(userId);
            ReceiveHelper helper = Services.getReceiveService().createHelper();
...
//
            json = helper.getReceiveTime().Desc().json().listPageJson(page, pageRows, new EachEntity2Map<Receive>() {
                @Override
